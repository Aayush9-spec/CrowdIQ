...
class CrowdSimulation:
    ...
    def __init__(self, venue: Any):
        ...
        self.lock = Lock()
        self.running = False
        self._event_phase = "PRE_MATCH"  # Internal property for phase change detection
    ...
    @event_phase.setter
    def event_phase(self, new_phase):
        if new_phase != self._event_phase:
            old_phase = self._event_phase
            self._event_phase = new_phase
            logging.info(f"Event phase changed from {old_phase} to {new_phase}")
            # Save snapshot to GCS on phase change
            self._trigger_snapshot(new_phase)

    def _trigger_snapshot(self, phase: str):
        """Triggers a state snapshot to GCS."""
        snapshot = {
            "timestamp": datetime.utcnow().isoformat(),
            "phase": phase,
            "venue": self.venue.name,
            "total_crowd": int(sum(self._exact_counts.values())),
            "zones": [
                {
                    "id": z.id,
                    "name": z.name,
                    "occupancy": z.current_count,
                    "wait_time": z.wait_time_minutes,
                }
                for z in self.venue.zones.values()
            ],
        }
        filename = f"snapshot_{phase.lower()}_{int(time.time())}"
        storage_service.save_snapshot(filename, snapshot)
        ...
        with self.lock:
            ...
            for zone in zones_list:
                ...
                monitoring_service.log_metric(
                    "zone_density", occupancy, {"zone_id": zone.id}
                )
