class CrowdEngine:
    ...
    def get_crowd_analytics(self):
        ...
        return {
            "summary": {
                ...
                "avg_occupancy": (
                    round(sum(z["occupancy_rate"] for z in zones) / len(zones), 1)
                    if zones
                    else 0
                ),
            },
            "top_crowded": sorted(
                highly_crowded, key=lambda x: x["occupancy_rate"], reverse=True
            )[:3],
            "recommendations": self._generate_recommendations(
                highly_crowded, low_crowded
            ),
        }
    ...
    def get_wait_time_predictions(self):
        ...
        phase = self.simulation.get_status()["phase"]

        predictions = []
        for zone in amenities:
            trend = "stable"

            # Phase-aware intelligent trend detection
            if phase == "PRE_MATCH":
                if zone["type"] == "gate":
                    trend = "increasing"
                elif zone["occupancy_rate"] > 40:
                    trend = "increasing"
            elif phase == "ONGOING":
                if zone["type"] == "gate":
                    trend = "decreasing"
                elif zone["occupancy_rate"] > 70:
                    trend = "increasing"
            elif phase == "HALFTIME":
                if zone["type"] in ["food", "restroom"]:
                    trend = "increasing"
            elif phase == "POST_MATCH":
                if zone["type"] == "gate":
                    trend = "increasing"
                elif zone["type"] in ["food", "restroom"]:
                    trend = "decreasing"
            ...
            predictions.append(
                {
                    ...
                    "predicted_wait_15m": zone["wait_time_minutes"]
                    + (
                        5
                        if trend == "increasing"
                        else -2 if trend == "decreasing" else 0
                    ),
                    "trend": trend,
                }
            )
    ...
    def get_venue_context_for_ai(self):
        ...
        for z in zones:
            zones_summary.append(
                f"{z['name']} ({z['type']}): {z['occupancy_rate']}% full, {z['wait_time_minutes']} min wait"
            )
